...
{
    public class StoryOtherTasksIssueMapper
    {
        ...
        public StoryOtherTasksIssueMapper(
            ...
            string storySummary)
        {
            ...
            var jiraIssue = new CreateIssueInfo
            {
                ...
                Summary = String.Format("{0} - {1}", geminiDevIssue.Title.TrimEnd(), storySummary),
                Description = storySummary,
                Type = type,
            };

            jiraIssue.Priority = geminiDevIssue.Priority;
            ...
            if (geminiDevIssue.DueDate.HasValue)
                jiraIssue.DueDate = geminiDevIssue.DueDate.Value;            

            //AffectedBuild
            var affectedBuild = geminiDevIssue.CustomFields.FirstOrDefault(x => x.Name == configurationSetup.Mapping.AFFECTEDBUILD_LABEL);
            if (affectedBuild != null && affectedBuild.FormattedData != "")
                jiraIssue.AffectVersions.Add(affectedBuild.FormattedData);

            //Assignee
            var owner = geminiDevIssue.CustomFields.FirstOrDefault(i => i.Name == "Owner");
            ...
            this.attachmentGetter.Execute(jiraIssue, geminiDevIssue.Attachments, configurationSetup.Gemini.ProjectUrl, configurationSetup.AttachmentDownloadedPath);

            //Load and map all gemini comments
            //commentMapper.Execute(configurationSetup, jiraIssue, geminiDevIssue);
            ...
            if (type == configurationSetup.Jira.StoryTypeCode)
            {
                ...
                //FixVersion
                var release = geminiDevIssue.CustomFields.FirstOrDefault(x => x.Name == configurationSetup.Mapping.RELEASE_KEY_LABEL);
                if (release != null && release.FormattedData != "")
                    jiraIssue.FixVersions.Add(release.FormattedData);

                //Development Line
                SetDevLine(geminiDevIssue, jiraIssue, configurationSetup.Mapping.DEV_LINE_MAPPING, configurationSetup.Mapping);

            }
            else
                LoadStorySubTaskCustomFields(jiraIssue, geminiDevIssue, configurationSetup.Mapping);

            //Watchers
            this.watchersEngine.Execute(jiraIssue, geminiDevIssue);

            //Load custom fields in common
            LoadCommonCustomFields(jiraIssue, geminiDevIssue, configurationSetup.Mapping, configurationSetup.Gemini.ErmPrefix);
            ...
            JiraTools.Parameters.MappingConfiguration mapping)
        {
            ...
            //JDE Code
            var jdeCode = geminiDevIssue.CustomFields.FirstOrDefault(i => i.Name == "JDE");
            if (jdeCode != null)
